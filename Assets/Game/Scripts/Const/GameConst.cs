public class Const_BattleEvent
{
    public const string event_fight_dead = "event_fight_dead";
    public const string event_fight_birth = "event_fight_birth";
    public const string event_fight_idle = "event_fight_idle";
    public const string event_fight_fight = "event_fight_fight";
    public const string event_fight_run = "event_fight_run";
    public const string event_fight_dizzy = "event_fight_dizzy";
}

public class Const_BattleAnim_Name
{
    public const string anim_birth = "anim_birth";
    public const string anim_dead = "anim_dead";
    public const string anim_idle = "anim_idle";
    public const string anim_run = "anim_run";
    public const string anim_attack1 = "anim_attack1";//普通攻击
    public const string anim_attack2 = "anim_attack2";//暴击
    public const string anim_dizzy = "anim_dizzy";//眩晕
}

public class Const_BattleAnim_State
{
    public const int birth = 1;
    public const int idle = 0;
    public const int run = 2;
    public const int attack1 = 3;
    public const int attack2 = 4;
    public const int dizzy = 5;
    public const int dead = 11;

}